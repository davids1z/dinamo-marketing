...
export default function Header({ title, subtitle, actions }: HeaderProps) {
  ...
  return (
    <header className="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 bg-white/80 backdrop-blur-xl sticky top-0 z-30 border-b border-gray-200">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative hidden md:block">
          ...
          <input
            ...
            className="bg-gray-50 border border-gray-200 rounded-xl pl-9 pr-4 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-dinamo-blue/50 focus:ring-2 focus:ring-dinamo-blue/10 w-52 transition-all"
          />
        </div>
        ...
        <div className="relative" ref={notifRef}>
          ...
          {showNotifs && (
            ...
            <div
              className="absolute top-full mt-2 bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden z-50 animate-fade-in"
              style={{ width: '380px', maxWidth: 'calc(100vw - 24px)', right: '-8px' }}
            >
              {/* Header */}
              <div className="px-5 py-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-dinamo-blue flex items-center justify-center">
                      ...
                      <div key={n.id} className={`border-l-[3px] ${cfg.accentColor} transition-colors ${!n.is_read ? 'bg-blue-50/50' : 'bg-transparent'}`}>
                        ...
                        {isExpanded && (
                          <div className="px-4 pb-3.5 ml-12 mr-4 animate-fade-in">
                            {n.link && (
                              <a href={n.link} className="text-[12px] text-dinamo-blue hover:underline block mb-2">
                                Pogledaj detalje →
                              </a>
                            )}
                            {!n.is_read && (

        ...
        <div key={i} className="card animate-pulse" style={{ animationDelay: `${i * 100}ms` }}>
          <div className="flex items-start justify-between">
            <div className="space-y-2 flex-1">
              <div className="h-3 w-20 bg-studio-surface-3 rounded" />
              <div className="h-8 w-28 bg-studio-surface-3 rounded" />
            </div>
            <div className="h-8 w-8 bg-studio-surface-3 rounded" />
          </div>
          <div className="h-3 w-32 bg-studio-surface-3 rounded mt-3" />
        </div>
      ))}
    </div>
  )
...
export function ChartSkeleton({ height = 300 }: { height?: number }) {
  return (
    <div className="card animate-pulse">
      <div className="h-4 w-48 bg-studio-surface-3 rounded mb-4" />
...
export function TableSkeleton({ rows = 5 }: { rows?: number }) {
  return (
    <div className="card animate-pulse">
      <div className="h-4 w-40 bg-studio-surface-3 rounded mb-4" />
      <div className="space-y-3">
        <div className="flex gap-4 pb-2 border-b border-studio-border-subtle">
          {[120, 80, 60, 80, 60].map((w, i) => (
            <div key={i} className="h-3 bg-studio-surface-3 rounded" style={{ width: w }} />
          ))}
        </div>
...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[300px] text-center">
      ...
      <p className="text-studio-text-secondary mb-4">{message}</p>
      {onRetry && (

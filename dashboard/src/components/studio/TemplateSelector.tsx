      ...
      <div className="grid grid-cols-2 gap-2">
        {ALL_TEMPLATES.map((t) => (
          <button
            key={t.id}
            onClick={() => onSelect(t)}
            className="text-left p-3 rounded-xl border border-gray-100 hover:border-dinamo-accent/30 hover:bg-dinamo-accent/5 transition-all group"
          >
            <div className="text-2xl mb-1.5">{t.icon}</div>
            <p className="text-[11px] font-semibold text-gray-700 group-hover:text-dinamo-accent-dark leading-tight">
              {t.name}
            </p>

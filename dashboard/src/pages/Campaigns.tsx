...
const columns = [
  ...
  { key: 'platform', header: 'Platforma', render: (row: CampaignRow) => <span className="text-gray-300 text-sm">{row.platform}</span> },
  { key: 'status', header: 'Status', render: (row: CampaignRow) => <StatusBadge status={row.status} /> },
  { key: 'budget', header: 'Budžet', render: (row: CampaignRow) => <span className="text-gray-300 font-mono">€{row.budget.toLocaleString()}</span> },
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-dinamo-dark-card rounded-xl border border-gray-800 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Sve kampanje</h2>
        ...
        <div className="bg-dinamo-dark-card rounded-xl border border-gray-800 p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-lg font-semibold text-white">A/B test rezultati: {abTestResults.campaign}</h2>
          </div>

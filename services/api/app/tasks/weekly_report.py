...
def _get_organic_summary(days=7):
    from app.database import SyncSessionLocal
    from app.models.analytics import PostMetric
...
def _generate_weekly_summary():
    ...
    top_posts = _get_top_posts(REPORT_PERIOD_DAYS)
    top_ads = _get_top_ads(REPORT_PERIOD_DAYS)
    organic = _get_organic_summary(REPORT_PERIOD_DAYS)
    paid = _get_paid_summary(REPORT_PERIOD_DAYS)
...
def generate_weekly_report(self):
    ...
    results = {"timestamp": run_ts, "report_generated": False, "report": None, "errors": []}
    ...
    try:
        ...
        logger.info("--- WEEKLY REPORT: %s to %s ---",
        ...
        try:
            ...
            logger.warning("Notification creation failed: %s", notif_exc)
